...
plt.show()
#0.002054,.004-.009999
finaltree=DecisionTreeClassifier(random_state=0,ccp_alpha=0.002054)
finaltree.fit(x_train,y_train)
pred1=finaltree.predict(x_train) 
from sklearn.metrics import accuracy_score
print('accurace:',accuracy_score(y_train,pred1))
pred_1= finaltree.predict(x_test) 
...
import matplotlib.pyplot as plt
plt.figure(figsize=(20,10))
tree.plot_tree(finaltree,rounded=True,filled=True,proportion=True)
plt.show()

sample=[18,43,240,5,3,3,3,10,3,2,3,3,1,25000,9600,3,0,2,2,2,2,2,0,1,1]

sample_df = pd.DataFrame([sample], columns=x.columns)
scaled_sample = st_x.transform(sample_df)
prediction = finaltree.predict(scaled_sample)
if(prediction==1):
    print("Yes employee will attriate")
else:
    print("No employee will attriate")